...
public class TKB_API {
    ...
    @GetMapping("normal")
    @CrossOrigin(origins = "*")
    public List<List<List<HocPhanDTO>>> normalMakeTKB(@RequestParam("dsMH") Set<String> dsMaMH, @RequestParam(name = "thu", defaultValue = "") Set<Byte> thu, @RequestParam(name = "gv", defaultValue = "") Set<String> gv) {
        if (thu.size() == 0) {
            if( gv.size() == 0){
            ...
            return normalMake(HocPhanDAO.dsHPbyMHAndGV(dsMaMH, gv));
        }
        if (gv.size() == 0) {
            System.out.println(3);
            return normalMake(HocPhanDAO.dsHPbyMH(dsMaMH,thu));
        }
        System.out.println(4);
        return normalMake(HocPhanDAO.dsHPbyMH(dsMaMH, thu, gv));
    }    @GetMapping("test")
    @CrossOrigin(origins = "*")
    public List test(@RequestParam("dsMH") Set<String> dsMaMH, @RequestParam(name = "thu", defaultValue = "") Set<Byte> thu, @RequestParam(name = "gv", defaultValue = "") Set<String> gv) {
        if (thu.size() == 0) {
            if( gv.size() == 0){
                System.out.println(1);
                return HocPhanDAO.dsHPbyMH(dsMaMH);
            }
            System.out.println(2);
            return HocPhanDAO.dsHPbyMHAndGV(dsMaMH, gv);
        }
        if (gv.size() == 0) {
            System.out.println(3);
            return HocPhanDAO.dsHPbyMH(dsMaMH,thu);
        }
        System.out.println(4);
        return HocPhanDAO.dsHPbyMH(dsMaMH, thu, gv);
    }
